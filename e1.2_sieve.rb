...
class Sieve
  ...
  def primes
    prime = []
    not_prime = []
    range.each do |counter|       #2
      range.each do |number|      #2, 3, 4, 5, 6, 7, etc
        if (number != counter) && (number % counter == 0) && !(not_prime.include?(number))
          not_prime << number
        end
      end
    end
    ...
    range.each do |digit|
      prime << digit if !(not_prime.include?(digit))
    end
    prime
  end
end
